...
namespace Common.Binding
{
    ...
    public class DProperty
    {
        ...
        static DProperty()
        {
            ...
            _store.Add(typeof(Models.MessageType), new BasicConverter("INT", p => toMessageType(p), p => (int)(Models.MessageType)p));
            
        }
        public static void Register(Type type, Converter converter) => _store[type] = converter;
        public static void RegisterEnum(Type type, Convert ToCsValue, Convert ToDbValue) => _store[type] = type.IsEnum ? new BasicConverter("INT", ToCsValue, ToDbValue) : throw new ArgumentException("Type must be Enum");
        private static Models.MessageType toMessageType(object p)
        {
            p = toEnum<Models.MessageType>(p);
            return p is int ? (Models.MessageType)(int)p : (Models.MessageType)p;
        }
        ...
        public static object toEnum<T>(object p) where T : struct
        {
