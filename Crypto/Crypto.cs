...
using System.Linq;
using System.Text;
using static Common.Crypto.Dyn;
namespace Common.Crypto
{
    ...
    public class AesCBC : Aes
    {
        ...
        public string Encrypt(string str)
        {
            ...
            return Encoding.UTF8.GetString(AES.ToBase64(igg));
        }
        public string Decrypt(string str)
        {
            var data = AES.FromBase64(Encoding.UTF8.GetBytes(str));
            var igg = Decrypt(data);
            return Encoding.UTF8.GetString(igg);
        }
...
namespace Common.Crypto1
{
    ...
    class Aes : ICrypto
    {
        ...
        public virtual byte[] Encrypt(byte[] data)
        {
            var Key = this.Key;
            var d = Key.Length;
            AddRoundKey(data, Key.slice(0, 16));
            var e = 16;
            for (; e < d - 16; e += 16)
            {
                SubBytes(data, Sbox);
                ShiftRows(data, ShiftRowTab);
                MixColumns(data);
                AddRoundKey(data, Key.slice(e, e + 16));
            }
            SubBytes(data, Sbox);
            ShiftRows(data, ShiftRowTab);
            AddRoundKey(data, Key.slice(e, d));
            return data;
        ...
        public virtual byte[] Decrypt(byte[] data)
        {
            var Key = this.Key;
            var d = Key.Length;
            AddRoundKey(data, Key.slice(d - 16, d));
            ShiftRows(data, ShiftRowTab_Inv);
            SubBytes(data, Sbox_Inv);
            for (d -= 32; d >= 16; d -= 16)
            {
                AddRoundKey(data, Key.slice(d, d + 16));
                MixColumns_Inv(data);
                ShiftRows(data, ShiftRowTab_Inv);
                SubBytes(data, Sbox_Inv);
            }
    ...
    class AesCBC : Aes
    {
        ...
        public override byte[] Encrypt(byte[] data)
        {
            ...
            for (var h = 0; h < e.Length / 16; h++)
            {
                ...
                f = blockXOR(g.slice(h * 16, h * 16 + 16), f);
                base.Encrypt(f);
                g = g.concat(f);
            }
            return g;
